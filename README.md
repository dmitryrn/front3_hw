...
- Отображение retry-кнопки при API-ошибке и вызов обработчика повтора
- Отображение ошибки без retry-кнопки, когда повтор недоступен
...
- Создание новых чатов с автоматически сгенерированными заголовками и сброс `isLoading`
- Удаление чатов и переключение на следующий доступный чат
- Сброс state при удалении последнего чата
- Сброс `isLoading` при удалении неактивного чата
- Переключение активного чата через `selectChat` со сбросом `isLoading`
...
- Обработка отмены генерации: удаление пустого placeholder и сброс loading
- Обработка отмены генерации: сохранение частичного ответа ассистента
...
- Нормализация `isLoading` в `false` при загрузке
- Реконструкция `currentChatMessages` из `messagesByChatId` при загрузке
